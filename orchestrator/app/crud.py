...
def create_persona(db: Session, persona: schemas.PersonaCreate):
    db_persona = models.Persona(name=persona.name, model_name=persona.model_name, system_prompt=persona.system_prompt, temperature=persona.temperature)
    db.add(db_persona)
    db.commit()
...
def update_persona(db: Session, persona_id: str, persona: schemas.PersonaCreate):
    db_persona = db.query(models.Persona).filter(models.Persona.id == persona_id).first()
    if db_persona:
        db_persona.name = persona.name
        db_persona.model_name = persona.model_name
